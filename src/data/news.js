// src/data/news.js
export const news = [
  {
    id: "1",
    title: "Transmisiones en vivo de partidos en YouTube",
    description: "¡No te pierdas los partidos del Club Evolución Deportiva transmitidos en vivo por nuestro canal de YouTube!",
    image: "/images/noticias/youtube.png",
    content: `
      📺 ¡Estamos emocionados de anunciar que podrás seguir los partidos de nuestro club en vivo a través de nuestro canal de YouTube!
      ⚽️ Acompaña a nuestros equipos en cada encuentro y no te pierdas ningún momento de la acción.
      🎥 ¡Suscríbete a nuestro canal y activa las notificaciones para no perderte nada!
      #PartidosEnVivo #Evoldep #ClubDeportivo #Futbol #SoyFelino #TransmisionesEnVivo
    `
  },
  {
    id: "2",
    title: "Graduación del entrenador Fernando Viafara",
    description: "Celebramos el logro de nuestro entrenador, Fernando Andrés Viafara, quien se ha graduado como Tecnólogo en Deporte.",
    image: "/images/noticias/grado.png",
    content: `
      ¡Tenemos un nuevo graduado en la casa tricolor! 🎓🧡🤍💙
      📚 Nuestro entrenador formador: Fernando Andrés Viafara Hurtado, ha alcanzado un hito importante: ¡se ha graduado como Tecnólogo en Deporte de la END! 🤩
      🌟 Su dedicación y pasión por el deporte son inspiración para todos nosotros.
      🏆 ¡Felicidades por este gran logro y gracias por guiar a nuestros atletas hacia el éxito!
      🎉 Resaltamos que en este momento se encuentra en la recta final en la misma institución universitaria para recibir el título de profesional en deporte. 🐯

      #TecnologoEndeporte #graduacion #Evoldep #Entrenador #OrgulloFloridano #SoyFelino #ClubDeportivo
    `
  },
  {
    id: "3",
    title: "Veedurías con Internacional F.C.",
    description: "Nuestros jugadores participaron en veedurías con el equipo profesional Internacional F.C. de Palmira.",
    image: "/images/noticias/veedurias.jpg",
    content: `
      ¡EVOLUCIÓN DEPORTIVA organizó veedurías con el equipo profesional Internacional F.C. de Palmira!
      ⚽️ Nuestros jugadores de las categorías sub 15, sub 14 y sub 13 participaron en estas importantes visorias con el scout Jorge Peralta. Se destacaron 6 jugadores que continuarán al siguiente filtro.
      🧡🤍💙 ¡Nos sentimos orgullosos y entusiasmados por este nuevo capítulo de nuestros deportistas! 💪

      #Veedurias #Evoldep #Futbol #Scout #SoyFelino #ClubDeportivo
    `
  },
  {
    id: "4",
    title: "¡Veedurías con el Club Deportivo Academia Alemana de Fútbol de Popayán! ⚽️",
    description: "Nuestros jugadores de las categorías sub-15, sub-14 y sub-13 participaron en estas importantes visorias con el scout y coordinador deportivo de esta destacada institución.",
    image: "/images/noticias/veedurias2.jpg",
    content: `
    En esta jornada, se destacaron especialmente:<br>
    - Un jugador nacido en 2009 ⚽<br>
    - Un jugador nacido en 2010 ⚽<br>
    - Tres jugadores nacidos en 2011 ⚽<br><br>
    
    Estos cinco talentos continuarán al siguiente filtro y se presentarán próximamente en Popayán en las instalaciones de esta prestigiosa entidad, reconocida por ser finalista en la Copa Las Américas sub-17 (2024) y protagonista en torneos nacionales sub-13, sub-15 y sub-17. Esta institución ya instala jugadores con convenio en el fútbol europeo y alemán.<br><br>
    
    Nuestra entidad extendió la invitación a este evento al Club Atlético Ortigal, permitiendo que coincidieran en la jornada y mostraran a sus jugadores destacados quedando 2 de ellos al siguiente filtro. Es importante mencionar que este club formará parte de nuestro proyecto en 2025.<br><br>
    
    ¡Nos sentimos orgullosos y entusiasmados por este nuevo capítulo para nuestros deportistas! ¡Vamos por más! 🚀<br>
    Orgullosamente EVOLUTIVOS 🚀. ¡Únete a la familia de tigrillos! 🐯
    `
  }
,  
  {
    id: "5",
    title: "¡Veedurías, esta vez con el Club Deportivo Fútbol Paz de Cali! ⚽️",
    description: "Nuestros jugadores de las categorías sub-15, sub-14 y sub-13 participaron en estas importantes visorias con el preparador físico de esta destacada institución.",
    image: "/images/noticias/veedurias3.jpg",
    content: `
      Aquí te dejo el párrafo mejorado con saltos de línea y algunos emojis para darle más dinamismo:<br><br>

      En esta jornada, se destacaron especialmente:<br>
      ⚽ Dos jugadores nacidos en 2009<br>
      ⚽ Dos jugadores nacidos en 2010<br>
      ⚽ Tres jugadores nacidos en 2011<br><br>

      Estos siete talentos continuarán al siguiente filtro y se presentarán en los próximos días en la sede deportiva La Z en Cali, instalaciones de esta prestigiosa entidad, reconocida por ser:<br>
      🏆 Campeona en diversas ocasiones de la liga vallecaucana de fútbol<br>
      🏆 Campeona en la Copa Las Américas sub-17<br>
      🏆 Protagonista en torneos nacionales sub-13 (actual semifinalista 2024), sub-15 y sub-17 (campeones 2014).<br><br>

      Esta institución ya ha ubicado jugadores en diferentes equipos del rentado nacional.<br>
      Nuestra entidad extendió la invitación a este evento al Club Atlético Ortigal, permitiendo que coincidieran en la jornada y mostraran a sus jugadores destacados.<br><br>

      ¡Nos sentimos orgullosos y entusiasmados por este nuevo capítulo para nuestros deportistas!<br>
      ¡Vamos por más!<br>
      Orgullosamente EVOLUTIVOS 🚀<br>
      ¡Únete a la familia de tigrillos! 🐯
    `
  },
  
  {
    id: "6",
    title: "Próximamente más noticias",
    description: "Mantente al tanto de las próximas noticias y actualizaciones del Club Evolución Deportiva.",
    image: "/images/noticias/proxi.png",
    content: `
      📢 ¡Próximamente más noticias y actualizaciones importantes!
      🧡🤍💙 Mantente al tanto de lo que sucede en nuestro club y no te pierdas las novedades.
      #Evoldep #Noticias #Próximamente #SoyFelino #ClubDeportivo
    `
  }
];

export function getNews() {
  return news;
}

export function getNewsById(id) {
  return news.find((item) => item.id === id) || null;
}
